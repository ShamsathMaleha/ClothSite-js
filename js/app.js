...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image card-img-top" src=${image}></img>
      ...
      <h6 class="card-text">Rating: ${product.rating.rate}</h6>
      <h6 class="card-text">${product.rating.count} Person rated this product</h6>
      <h2 class="card-text mb-4">Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-info"><i class="far fa-shopping-cart"></i>Add to cart</button>
      <button id="details-btn" onclick="details(${product.id})" class="btn btn-warning" 
      type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">Details</button></div>
      </div>
